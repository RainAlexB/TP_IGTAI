...
#define WIDTH 800
#define HEIGHT 600
#define MESHFILE "obj_files/deer.obj"
...
Scene* initScene4(){
    Scene *scene = initScene();
    setCamera(scene, point3(6,4,6), vec3(0,1,0), vec3(0,1,0), 90, (float)WIDTH/(float)HEIGHT);
    setSkyColor(scene, color3(0.2, 0.2, 0.7));
    Material mat;
    mat.diffuseColor = color3(0.301, 0.034, 0.039);
    mat.specularColor  = color3(1.0, 0.992, 0.98);
    ...
    for(int i =0; i<600; i++){
        addObject( scene, initSphere(point3(1+rand()%650/100.0,rand()%650/100.0,1+rand()%650/100.0),.05+rand()%200/1000.0, mat_lib[rand()%10]));
        }
    return scene;
}


void recurseSphere(point3 center, float radius, vec3 dir1, vec3 dir2, Scene * scene, int depth){
    if(depth <= 0) return;
    vec3 dir3 = cross(dir1, dir2);
    addObject( scene, initSphere(center, radius, mat_lib[0]));
    recurseSphere(center + 1.5f*radius*dir1, 0.45f*radius, dir1, dir2,  scene, depth-1);
    recurseSphere(center + 1.5f*radius*dir2, 0.45f*radius, dir2, dir1,  scene, depth-1);
    recurseSphere(center - 1.5f*radius*dir2, 0.45f*radius, -dir2, dir1,  scene, depth-1);
    recurseSphere(center + 1.5f*radius*dir3, 0.45f*radius, dir3, dir1,  scene, depth-1);
    recurseSphere(center - 1.5f*radius*dir3, 0.45f*radius, -dir3, dir1,  scene, depth-1);
}
...
Scene* initScene5(){
    ...
    recurseSphere(point3(3, 3, 3), 0.8, vec3(0,1,0), vec3(1,0,0),  scene, 4);
...
Scene * initScene6() {
    Scene *scene = initScene();
    setCamera(scene, point3(3,1.5,2), vec3(0,1,0), vec3(0,1,0), 90, (float)WIDTH/(float)HEIGHT);
    setSkyColor(scene, color3(0.2, 0.2, 0.7));
    Material mat;
    mat.diffuseColor = color3(0.301, 0.034, 0.039);
    mat.specularColor  = color3(1.0, 0.992, 0.98);
    ...
    addObject(scene, initPyramid(0.75, 1, point3(0, 0, 0), mat, vec3(0)));
    addObject(scene, initPyramid(1, 0.125, point3(-0.5, 0, -1.25), mat_lib[4], vec3(0, 90, 45)));
...
Scene * initScene7() {
    Scene *scene = initScene();
    setCamera(scene, point3(4,2,4), vec3(0,1,0), vec3(0,1,0), 90, (float)WIDTH/(float)HEIGHT);
    // setCamera(scene, point3(2,2,2), vec3(0,1,0), vec3(0,1,0), 90, (float)WIDTH/(float)HEIGHT);
    setSkyColor(scene, color3(0.1f, 0.3f, 0.5f));
    Material mat;
    mat.IOR = 1.3;
    mat.roughness = 0.1;
...
int main(int argc, char *argv[]) {
    printf("IGTAI RayTracer project\n");
